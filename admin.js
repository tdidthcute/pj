      ...
      <div style="overflow-x:auto">
        <table class="tbl">
          ...
          <tbody>
            ${
              volunteers.length === 0
                ? `<tr><td colspan="8" style="text-align:center;padding:2rem">📭 Chưa có tình nguyện viên nào</td></tr>`
                : volunteers
                    .map(
                      (v) => `
                  <tr>
                    <td><div style="font-weight:600">${escapeHtml(v.name)}</div></td>
                    <td><span class="sbadge" style="background:#E8F0FF;color:var(--blue);padding:0.25rem 0.6rem">${escapeHtml(v.role)}</span></td>
                    <td style="font-size:0.82rem">${escapeHtml(v.area || "Chưa cập nhật")}</td>
                    <td style="font-size:0.82rem">${escapeHtml(v.phone || "—")}</td>
                    <td><span style="font-family:var(--font-mono);font-weight:700;background:var(--cream);padding:0.2rem 0.6rem;border-radius:20px">${v.missions || 0}</span></td>
